...
namespace QRCode
{
    ...
    {
        #region Dictionary
        public static Dictionary<int, int> GF256Mapping = new Dictionary<int, int>()
        {
        ...
        public static int XorTwoCoefficients(int n1, int n2)
        {
            return GF256Mapping.FirstOrDefault(x => x.Value == ((GF256Mapping[n1] ^ GF256Mapping[n2]) % 256)).Key;
        }

        public static int[] MultiplyTwoPolynoms(int[] p1, int[] p2)
        {
            ...
            for (int i = 0; i < p1.Length; i++){
                for (int j = 0; j < p2.Length; j++){
                    if (res[i + j] == -1)
                        res[i + j] = (p1[i] + p2[j]) % 256 + (p1[i] + p2[j]) / 256;
                    else
                        res[i + j] = XorTwoCoefficients((p1[i] + p2[j]) % 256 + (p1[i] + p2[j]) / 256, res[i + j]);
                }
            }
        ...
        public static int[] DivideTwoPolynoms(int[] p1, int[] p2)
        {
            int[] res = new int[1];
            return res;
        }

        public static int[] GenerateGeneratorPolynomial(int len)
        {
            int[] res = new int[] { 0, 0 };

            for (int i = 1; i < len; i++){
                res = MultiplyTwoPolynoms(res, new int[] { i, 0 });
            }

            return res;
        }

        public static string CoefficientToString(int[] p)
        {
            string str = "Polynom Coefficients(the nums represent n, when the actual coefficents are 2^n): ";
